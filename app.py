...
sample_files = {
    "Bitcoin (BTC)": "data/bitcoin.csv",
    "Ethereum (ETH)": "data/ethereum.csv",
    "Solana (SOL)": "data/solana.csv",
}

data_source = st.sidebar.radio(
    "Choose how to load data:", ["Use a sample coin", "Upload my own CSV"]
)
...
if data_source == "Use a sample coin":
    ...
    st.sidebar.markdown(
        "Don't have a CSV? Download one free from "
        "[CoinGecko](https://www.coingecko.com) — go to any coin's page → "
        "**Historical Data** → download CSV, then upload it below."
    )
    uploaded_file = st.sidebar.file_uploader(
        "Drag & drop a CSV file here (from your laptop)", type=["csv"]
    )
...
if uploaded_file is not None:
    ...
    volume_col = st.selectbox(
        "Select the Volume column (optional)", options=[None] + columns, index=0
    )

    # Load and process data
    if hasattr(uploaded_file, "seek"):
        uploaded_file.seek(0)  # Reset file pointer (only needed for real uploads)
    ...
    if df is not None and not df.empty:
        ...
        prophet_df = df[["date", "price", "volume"]].rename(
            columns={"date": "ds", "price": "y"}
        )
        prophet_df["ds"] = pd.to_datetime(
            prophet_df["ds"], errors="coerce", utc=True
        ).dt.tz_localize(None)
        ...
        future["volume"] = (
            prophet_df["volume"].mean() if "volume" in prophet_df.columns else 0
        )
        ...
        forecast_match = forecast[
            forecast["ds"].dt.normalize() == latest_date_normalized
        ]
        if not forecast_match.empty:
            latest_forecast = forecast_match["yhat"].iloc[0]
            scaling_factor = (
                latest_price / latest_forecast if latest_forecast != 0 else 1
            )
        ...
        historical_data = prophet_df.merge(
            historical_forecast[["ds", "yhat"]], on="ds", how="inner"
        )
        ...
        fig1.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name="Upper Bound",
                line=dict(color="#e5e80e", dash="dash"),
                fill=None,
            )
        )
        fig1.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name="Lower Bound",
                line=dict(color="#cfe40d", dash="dash"),
                fill="tonexty",
                opacity=0.2,
            )
        )
        ...
        if not historical_data.empty:
            historical_data["residuals"] = (
                historical_data["y"] - historical_data["yhat"]
            )
        ...
        if not future_data.empty:
            forecast_monthly = (
                future_data.groupby("year_month")["yhat"].mean().reset_index()
            )
            monthly_trends = pd.concat(
                [
                    monthly_trends.rename(
                        columns={"price": "value", "year_month": "date"}
                    ),
                    forecast_monthly.rename(
                        columns={"yhat": "value", "year_month": "date"}
                    ),
                ]
            ).reset_index(drop=True)
        else:
            monthly_trends = monthly_trends.rename(
                columns={"price": "value", "year_month": "date"}
            )

        monthly_trends["moving_avg"] = (
            monthly_trends["value"].rolling(window=3, min_periods=1).mean()
        )
        fig2 = go.Figure()
        fig2.add_trace(
            go.Scatter(
                x=monthly_trends["date"],
                y=monthly_trends["value"],
                mode="lines+markers",
        ...
        fig2.add_trace(
            go.Scatter(
                ...
                line=dict(color="#ff7300", dash="dash"),
            )
        )
        fig2.add_annotation(
            x=latest_date,
            y=latest_price,
            text=f"Actual: ${format_number(latest_price)}",
            showarrow=True,
            arrowhead=2,
            ax=20,
            ay=-30,
        ...
        daily_trends["color"] = daily_trends["mean"].apply(
            lambda x: f'rgba(136, 132, 216, {x/daily_trends["mean"].max()})'
        )
        fig3 = go.Figure()
        fig3.add_trace(
            go.Bar(
                x=daily_trends["day_of_week"],
                y=daily_trends["mean"],
                name="Average Price",
                marker_color=daily_trends["color"],
        ...
        if not future_years.empty:
            forecast_yearly = (
                future_years.groupby(future_years["ds"].dt.year)["yhat"]
                .mean()
                .reset_index()
            )
            forecast_yearly.columns = ["year", "value"]
            yearly_trends = pd.concat(
                [yearly_trends.rename(columns={"price": "value"}), forecast_yearly]
            ).reset_index(drop=True)
        else:
        ...
        if not future_years.empty:
            ...
            if not year_data.empty:
                ...
                fig4.add_trace(
                    go.Scatter(
                        x=[forecast_max_year, forecast_max_year],
                        y=[forecast_lower, forecast_upper],
                        mode="lines",
                        name=f"{forecast_max_year} Confidence Interval",
                        line=dict(color="#ff7300", dash="dash"),
                    )
                )
        ...
        fig4.add_annotation(
            x=latest_date.year,
            y=latest_price,
            text=f"Actual {latest_date.year}: ${format_number(latest_price)}",
            showarrow=True,
            arrowhead=2,
            ax=20,
            ay=-30,
        ...
        forecast_latest = (
            forecast_current["yhat"].iloc[0]
            if not forecast_current.empty
            else latest_price
        )
        ...
        forecast_next_month = (
            forecast_next["yhat"].iloc[0]
            if not forecast_next.empty
            else forecast_latest
        )
        ...
        if not historical_data.empty:
            model_performance_summary = (
                f" The Prophet model demonstrates {quality.lower()} performance with an R² of "
                f"{metrics['R²']:.3f}."
            )
        ...
        st.error(
            "Failed to load or process the dataset. Please ensure it contains valid date and price columns."
        )
else:
